...
| G5b | **Event-Driven Ansible (EDA) integration** — connect AWX to an EDA Controller instance; `/api/v2/eda/status/`, `/api/v2/eda/activations/`, and `/api/v2/eda/activations/start/` now provide shared backend status/activation context plus admin-gated activation create/start/poll behavior, with encrypted token/SSL/timeout/path settings and live activation reads when configured; workflow EDA metadata nodes can be created/edited/saved in the visualizer, create/start/poll controller activations at runtime, expose EDA metadata/actions/events/status to downstream nodes, and route failed activation checks through workflow failure paths; richer EDA log UI/native activation CRUD remain | High | 🔄 |
...
| I7 | **EDA node parity** — represent EDA controller connections, rulebook activations, and event source status as first-class workflow visualizer node choices; backend EDA status/activation facade, encrypted controller auth settings, admin-gated activation create/start endpoint, workflow AI planning context, persisted workflow node metadata, visualizer create/edit/save paths, workflow launch preflight, runtime controller create/start/poll status sync, success/failure path handling, and downstream artifact propagation are wired, while richer EDA log UI and native activation CRUD remain | High | 🔄 |
