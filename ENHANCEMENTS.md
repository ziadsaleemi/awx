...
| G5b | **Event-Driven Ansible (EDA) integration** — connect AWX to an EDA Controller instance; `/api/v2/eda/status/` and `/api/v2/eda/activations/` now provide shared backend status/activation context for overview and workflow AI planning; workflow EDA metadata nodes can be created/edited/saved in the visualizer; launch/status handling remains | High | 🔄 |
...
| I7 | **EDA node parity** — represent EDA controller connections, rulebook activations, and event source status as first-class workflow visualizer node choices; backend EDA status/activation facade, workflow AI planning context, persisted workflow node metadata, and visualizer create/edit/save paths are wired, while workflow launch/status handling remains | High | 🔄 |
...
| AP14 | **G5 workflow parity was over-marked done** — the workflow visualizer now has a human-approved AI plan preview/apply path for deterministic AWX, Terraform, approval, and EDA metadata nodes, and EDA controller/activation context is available to the planner, but EDA launch/status handling, persisted AI node type, and runtime AI execution-plan generation remain incomplete | Documentation Drift | High | 🔄 |
