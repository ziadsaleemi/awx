...
| G6e | **Gatekeeper Policy Manager parity view** — Policy as Code → Gatekeeper now has a read-only GPM-inspired view backed by the configured Kubernetes API: explicit cluster/config/auth error states, ConstraintTemplates with target/Rego/lib/schema/status metadata, Constraints sorted by violations with enforcement/match/parameters/status detail, Config CRD sync/readiness views, violations search, and downloadable JSON report; still needs full kubeconfig multi-context selection, per-pod status drill-down, richer violation pagination/sort controls, and template/constraint deep links | High | 🔄 |
...
| AP51 | **OPA management is not Gatekeeper Policy Manager parity** — Plain OPA live module management is covered by G6d and the first read-only Gatekeeper inventory/report view is covered by G6e, but parity is still incomplete: AWX still needs full kubeconfig multi-context selection, deeper ConstraintTemplate/Constraint drill-downs, richer violation pagination/sort/reporting, and governed Gatekeeper write actions tracked in G6e/G6f | Stub / Approximation | High | 🔄 |
