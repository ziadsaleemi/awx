...
| G6e | **Gatekeeper Policy Manager parity view** — Policy as Code → Gatekeeper now has a read-only GPM-inspired view backed by the configured Kubernetes API: explicit cluster/config/auth error states, ConstraintTemplates with target/Rego/lib/schema/status metadata and drill-down, Constraints sorted by violations with enforcement/match/parameters/status drill-down, Config CRD sync/readiness drill-down, query-backed violation search/sort/limit controls, and downloadable JSON report; still needs full kubeconfig multi-context selection, route-backed permalinks/deep links, and governed write actions tracked in G6f | High | 🔄 |
...
| AP51 | **OPA management is not Gatekeeper Policy Manager parity** — Plain OPA live module management is covered by G6d, the read-only Gatekeeper inventory/report/drill-down view is covered by G6e, and governed Gatekeeper preview/dry-run/apply/delete/rollback/server-side apply plus AI-assisted manifest authoring is covered by G6f, but parity is still incomplete: AWX still needs full kubeconfig multi-context selection, route-backed Gatekeeper permalinks, and true paginated violation tables for very large clusters | Stub / Approximation | High | 🔄 |
