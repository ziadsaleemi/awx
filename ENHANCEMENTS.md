...
| G5b | **Event-Driven Ansible (EDA) integration** — connect AWX to an EDA Controller instance; `/api/v2/eda/status/` and `/api/v2/eda/activations/` now provide shared backend status/activation context for overview and workflow AI planning, with encrypted token/SSL/timeout/path settings and live activation reads when configured; workflow EDA metadata nodes can be created/edited/saved in the visualizer, sync controller activation status at runtime, expose EDA metadata/status to downstream nodes, and route failed activation checks through workflow failure paths; activation create/start/poll logs remain | High | 🔄 |
...
| I7 | **EDA node parity** — represent EDA controller connections, rulebook activations, and event source status as first-class workflow visualizer node choices; backend EDA status/activation facade, encrypted controller auth settings, workflow AI planning context, persisted workflow node metadata, visualizer create/edit/save paths, workflow launch preflight, runtime controller status sync, success/failure path handling, and downstream artifact propagation are wired, while EDA activation create/start and log/event polling remain | High | 🔄 |
...
| AP14 | **G5 workflow parity was over-marked done** — the workflow visualizer now has a human-approved AI plan preview/apply path for deterministic AWX, Terraform, approval, and EDA metadata nodes, EDA controller/activation context is available to the planner, and EDA metadata nodes sync configured controller activation status at workflow runtime, but EDA activation create/start/log polling, persisted AI node type, and runtime AI execution-plan generation remain incomplete | Documentation Drift | High | 🔄 |
