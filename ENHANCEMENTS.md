...
| G5b | **Event-Driven Ansible (EDA) integration** — connect AWX to an EDA Controller instance; `/api/v2/eda/status/` and `/api/v2/eda/activations/` now provide shared backend status/activation context for overview and workflow AI planning; persisted workflow EDA node launch/status handling remains | High | 🔄 |
...
| I6 | **Workflow plan apply** — upgrade the workflow AI suggestion panel from plain text guidance to a structured plan that can add deterministic, approval, and Terraform/existing AWX executable nodes to the visualizer with preview before save; the panel now requests strict JSON, resolves proposed nodes against visible Unified Job Templates, blocks unresolved/EDA/runtime-AI nodes before apply, and adds approved unsaved nodes/edges into the visualizer for normal Save persistence | High | 🔄 |
| I7 | **EDA node parity** — represent EDA controller connections, rulebook activations, and event source status as first-class workflow visualizer node choices; backend EDA status/activation facade and workflow AI planning context are wired, while persisted workflow node creation plus launch/status handling remain | High | 🔄 |
...
| AP14 | **G5 workflow parity was over-marked done** — the workflow visualizer now has a human-approved AI plan preview/apply path for deterministic AWX, Terraform, and approval nodes, and EDA controller/activation context is available to the planner, but persisted EDA nodes, persisted AI node type, and runtime AI execution-plan generation remain incomplete | Documentation Drift | High | 🔄 |
