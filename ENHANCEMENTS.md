...
| G6b | **Policy status and tester UI** — Policy as Code has its own sidebar section with Overview, Policy Modules, Policy Tester, and Smoke Test pages; Overview shows OPA enforcement status, server URL, managed bundle size, and registered decision paths; Policy Tester posts sample JSON to the real OPA Data API adapter; Settings → OPA Connection keeps server/auth/timeout configuration only | Medium | ✅ |
...
| G6e | **Gatekeeper Policy Manager parity view** — add a Gatekeeper tab inspired by `sighupio/gatekeeper-policy-manager`: kubeconfig/context selection, ConstraintTemplates with Rego/libs/schema/status/related constraints, Constraints sorted by violations with enforcement mode/match/parameters/per-pod status, Config CRD views, violations search/sort/pagination, downloadable report, and explicit cluster/auth/error states | High | ⬜ |
...
| AP51 | **OPA management is not Gatekeeper Policy Manager parity** — Plain OPA live module management is now covered by G6d, but Gatekeeper parity is still incomplete: AWX still needs multi-context cluster selection, ConstraintTemplate Rego/libs/schema/status/related-constraint views, Constraint violation search/sort/reporting with enforcement/match/parameter/status detail, Config CRD views, kubeconfig/auth error handling, and governed Gatekeeper write actions tracked in G6e/G6f | Stub / Approximation | High | 🔄 |
