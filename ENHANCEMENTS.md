...
| AP51 | **OPA management is not Gatekeeper Policy Manager parity** — Plain OPA live module management is covered by G6d, the read-only Gatekeeper inventory/report/drill-down view is covered by G6e, and governed Gatekeeper preview/dry-run/apply/delete/rollback/server-side apply is covered by G6f, but parity is still incomplete: AWX still needs full kubeconfig multi-context selection, route-backed Gatekeeper permalinks, true paginated violation tables for very large clusters, and AI-assisted Gatekeeper authoring | Stub / Approximation | High | 🔄 |
