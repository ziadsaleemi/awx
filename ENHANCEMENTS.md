...
| AP15 | **AI resource creation is not end-to-end** — typed backend plan/preview/apply now exists for projects, job templates, workflow job templates, inventories, smart inventories, constructed inventories, and catalog items through `/api/v2/ai/resource_actions/`, but frontend approval UX, inventory sources, schedules, credential references, role assignments, and project-backed playbook/role file authoring are still incomplete | Stub / Approximation | High | 🔄 |
