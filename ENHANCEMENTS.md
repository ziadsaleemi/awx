...
| AP26 | **OPA guardrail scope is still missing human approval/destructive-action policy paths** — shared OPA launch enforcement now runs before Job Template, Workflow Job Template, Terraform Job Template, System Job Template, and Catalog deployment launches; launch inputs are secret-redacted and include source/template/inventory/credential/extra-var-key context; Policy as Code settings now include an editable `OPA_POLICY_BUNDLE` source field surfaced in OPA status. Remaining gap: first-class human-approval/destructive-action enforcement for future AI workflow nodes | Stub / Approximation | High | 🔄 |
