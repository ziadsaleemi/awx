...
| G6f | **OPA/Gatekeeper write governance** — extend beyond GPM's read-only surfaces with RBAC/OPA-gated apply/delete actions for OPA modules, ConstraintTemplates, Constraints, and Configs; require preview/diff, dry-run when available, Activity Stream audit, rollback plan, and AI-assisted policy generation scoped to visible AWX/Gatekeeper resources | High | ⬜ |
...
| AP51 | **OPA management is not Gatekeeper Policy Manager parity** — Plain OPA live module management is covered by G6d and the read-only Gatekeeper inventory/report/drill-down view is covered by G6e, but parity is still incomplete: AWX still needs full kubeconfig multi-context selection, route-backed Gatekeeper permalinks, true paginated violation tables for very large clusters, and governed Gatekeeper write actions tracked in G6f | Stub / Approximation | High | 🔄 |
