...
| G6f | **OPA/Gatekeeper write governance** — Policy as Code → Gatekeeper now has a governed manifest path for ConstraintTemplates, Constraints, and Config CRDs: preview shows target/diff/rollback without Kubernetes writes, dry-run/apply use the configured Kubernetes API, dry-run/apply are checked through the `awx/gatekeeper_resource/allow` OPA decision path, and write attempts create Activity Stream audit entries with target/checksum/rollback metadata; still needs governed delete actions, server-side apply/field-manager support, richer rollback execution UX, and AI-assisted policy generation scoped to visible AWX/Gatekeeper resources | High | 🔄 |
...
| AP51 | **OPA management is not Gatekeeper Policy Manager parity** — Plain OPA live module management is covered by G6d, the read-only Gatekeeper inventory/report/drill-down view is covered by G6e, and the first governed Gatekeeper preview/dry-run/apply path is covered by G6f, but parity is still incomplete: AWX still needs full kubeconfig multi-context selection, route-backed Gatekeeper permalinks, true paginated violation tables for very large clusters, governed delete/rollback execution, and AI-assisted Gatekeeper authoring | Stub / Approximation | High | 🔄 |
