...
| I6 | **Workflow plan apply** — upgrade the workflow AI suggestion panel from plain text guidance to a structured plan that can add deterministic, approval, Terraform/existing AWX executable nodes, and EDA metadata nodes to the visualizer with preview before save; the panel now requests strict JSON, resolves proposed nodes against visible Unified Job Templates and EDA context, blocks unresolved/runtime-AI nodes before apply, and adds approved unsaved nodes/edges into the visualizer for normal Save persistence | High | 🔄 |
...
| AP14 | **G5 workflow parity was over-marked done** — the workflow visualizer now has a human-approved AI plan preview/apply path for deterministic AWX, Terraform, approval, and EDA metadata nodes, EDA controller/activation context is available to the planner, and EDA metadata nodes create/start/poll configured controller activations at workflow runtime, but native EDA activation CRUD/log UI, persisted AI node type, and runtime AI execution-plan generation remain incomplete | Documentation Drift | High | 🔄 |
