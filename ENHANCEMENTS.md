...
| AP44 | **External automation smoke was CLI-only** — Live EDA/OPA parity checks were repeatable from `awx-manage` but invisible in the AWX UI/API, so admins could not verify controller/API wiring from Settings. `/api/v2/external_automation/check/` now exposes the same admin-only smoke runner, and Settings → Event-Driven Ansible plus Settings → Policy as Code show a live EDA/OPA smoke panel that runs EDA read status with OPA policy sync, allow evaluation, deny evaluation, and temporary policy cleanup | Implementation Bug | High | ✅ |
