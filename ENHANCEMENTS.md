...
| G6e | **Gatekeeper Policy Manager parity view** — Policy as Code → Gatekeeper now has a GPM-inspired view backed by the configured Kubernetes API: explicit cluster/config/auth error states, named Kubernetes context selection through the Gatekeeper context map, ConstraintTemplates with target/Rego/lib/schema/status metadata and drill-down, Constraints sorted by violations with enforcement/match/parameters/status drill-down, Config CRD sync/readiness drill-down, query-backed violation search/sort/limit controls, and downloadable JSON report; governed write actions are tracked in G6f, while route-backed permalinks/deep links and true paginated violation tables still remain | High | 🔄 |
...
| AP51 | **OPA management is not Gatekeeper Policy Manager parity** — Plain OPA live module management is covered by G6d, the Gatekeeper inventory/report/drill-down/context selector view is covered by G6e, and governed Gatekeeper preview/dry-run/apply/delete/rollback/server-side apply plus AI-assisted manifest authoring is covered by G6f, but parity is still incomplete: AWX still needs route-backed Gatekeeper permalinks and true paginated violation tables for very large clusters | Stub / Approximation | High | 🔄 |
