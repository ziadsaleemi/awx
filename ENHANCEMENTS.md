...
| G5c | **AI-driven node type** — persisted `ai_task` workflow nodes now delegate execution-plan generation to the configured AI provider at runtime, store provider/model/response/plan/error artifacts for downstream nodes, and route provider failures through workflow failure paths; approval metadata is captured, while approval-gated execution of generated plans remains | Medium | 🔄 |
...
| I6 | **Workflow plan apply** — upgrade the workflow AI suggestion panel from plain text guidance to a structured plan that can add deterministic, approval, Terraform/existing AWX executable nodes, EDA metadata nodes, and persisted AI task nodes to the visualizer with preview before save; the panel now requests strict JSON, resolves proposed nodes against visible Unified Job Templates and EDA context, and adds approved unsaved nodes/edges into the visualizer for normal Save persistence | High | 🔄 |
