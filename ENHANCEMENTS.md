...
| AP15 | **AI resource creation is not end-to-end** — typed backend plan/preview/apply now exists for projects, job templates, workflow job templates, inventories, smart inventories, constructed inventories, inventory sources, schedules, credential references for job templates and inventory sources, and catalog items through `/api/v2/ai/resource_actions/`, and the masthead assistant now exposes a frontend preview/apply approval path; role assignments, richer per-page entry points, schedule launch credential references, workflow-node credential references, and project-backed playbook/role file authoring are still incomplete | Stub / Approximation | High | 🔄 |
